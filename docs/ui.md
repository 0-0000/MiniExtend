...
## <code style="color:green;">MUI</code> 作用域
MiniExtend MUI 前身为 `Customui` ，但以面相对象方式描述界面与元件，且支持 [MiniExtend Object](./object.html) 。  

大多数 `MUI` 下的函数都会包含一个 `[, playerid]` 参数，它表示要响应该操作的玩家 id ，该参数不会被检查，如果 `playerid` 的布尔值为 `false` ，则以 `objid` 代替。  
`MUI` 下的所有未表明返回值的函数或方法，一律返回 `true` 表示<u title="调用 API 成功不一定代表函数正确工作">调用 API 成功</u>，返回 `false` 表示<u title="只要有一次调用 API 失败">失败</u>。  
...
## `MUI.UIView` 类
一个 `MUI.UIView` 对象代表了一个 UI 界面，以下简称 `UIView` 。  

### 构造函数
- 使用 `MUI.UIView:new(uiid);` 来构造一个 `UIView` 对象。  
...
- `onShow`:`{function | nil}` 当 UI 界面显示时回调的函数，你可以修改它来改变 UI 界面显示时回调的函数。  
- `onHide`:`{function | nil}` 当 UI 界面隐藏时回调的函数，类似。  
...
## `MUI.Element` 类
一个 `MUI.Element` 对象代表一个 UI 元件，以下简称 `Element` 。  
...
## `MUI.Texture` 类
一个 `MUI.Texture` 对象代表一个 UI 图片元件，以下简称 `Texture` 。  
使用 `MUI.Texture:new(uiview, elementid);` 来构造一个 `Texture` 对象。  
...
## `NUI.Button` 类
一个 `MUI.Button` 对象代表一个 UI 图片元件，以下简称 `Button` 。  
使用 `MUI.Button:new(uiview, elementid);` 来构造一个 `Button` 对象。  

### 属性
- `onPress`:`{function | nil}` 按钮被按下时调用的函数。  
> 默认为 `nil` ，表示在事件发生时无需回调。  
> 你可以修改它来改变回调的函数。  
> 与 `Event:connect([[$ui.onPress]], onPress, uiid)` 类似，会传递 `param` 作为参数。  
> `param` 额外包含 `element` 键，表示这个 `Button` 对象。  
- `onClick`:`{function | nil}` 与 `onPress` 类似，但是在按钮被点击时回调。  

## `MUI.Label` 类
一个 `MUI.Label` 对象代表一个 UI 文字元件，以下简称 `Label` 。  
使用 `MUI.Label:new(uiview, elementid);` 来构造一个 `Label` 对象。  
...
## `MUI.EditBox` 类
一个 `MUI.EditBox` 对象代表一个 UI 输入框元件，以下简称 `EditBox` 。  
使用 `MUI.EditBox:new(uiview, elementid);` 来构造一个 `EditBox` 对象。  

### 属性
- `onLostFocus`:`{function | nil}` 输入框失去焦点时调用的函数。  
> 默认为 `nil` ，表示在事件发生时无需回调。  
> 你可以修改它来改变回调的函数。  
> 与 `Event:connect([[$ui.onLostFocus]], onLostFocus, uiid)` 类似，会传递 `param` 作为参数。  
> `param` 额外包含 `element` 键，表示这个 `Button` 对象。  
> 提示: `param` 中 `content`:`{string}` 是最有用的参数，表示输入框输入的信息。  
> 输入框的内容会先被和谐然后才会回调，这意味着 `content` 是和谐后的内容。  
> 注意 `setText()` 方法不会是文本被和谐。  
...
local label_id = [[]]
local button_id = [[]]
uiview = MUI.UIView:new(uiview_id)
